mod parse {
    ...
    */
    ...
    pub fn gen_insn_set(path: &str) -> Vec<(&str, Vec<&str>)> {
        let stream = fs::read_to_string(path).expect("Invalid path");
...
fn main() {
    ...
    let data = parse::gen_insn_set(filename);
}